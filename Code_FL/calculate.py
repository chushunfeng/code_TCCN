...
def para_root(para_a):
    res_para = copy.deepcopy(para_a)
    for i in para_a.keys():
        
        res_para[i] = copy.deepcopy(pow(para_a[i], 0.5))
    # print('Para root:', para_a[i],pow(para_a[i], 0.5))
    return res_para

def para_divide(para_a, para_b):
    
    res_para = copy.deepcopy(para_a)
    for i in para_a.keys():
        
        res_para[i] = copy.deepcopy(para_a[i]/para_b[i])
    # print('Para divide:', para_a[i], para_a[i]/para_b[i])
    return res_para

def para_pow(para_a):
    res_para = copy.deepcopy(para_a)
    if isinstance(para_a,dict):
        for i in para_a.keys():
            if len(res_para[i])==1:
                res_para[i]=copy.deepcopy(pow(para_a[i],2))
            else:
                for j in range(len(res_para[i])):
                    res_para[i][j]=copy.deepcopy(pow(para_a[i][j],2))
    else:
        res_para=copy.deepcopy(pow(para_a, 2))
    return res_para

def multipl_cons(para_a, lambda_):
    res_para = copy.deepcopy(para_a)
    if isinstance(para_a,dict):
        for i in para_a.keys():
            if len(res_para[i])==1:
                res_para[i]=copy.deepcopy(lambda_*para_a[i])
            else:
                for j in range(len(res_para[i])):
                    res_para[i][j]=copy.deepcopy(lambda_*para_a[i][j])
    else:
        res_para=copy.deepcopy(lambda_*para_a)
    return res_para
...
def add_cons(params_a, params_b):
    w = copy.deepcopy(params_a)
    for k in w.keys():
            w[k] += params_b
    return w
